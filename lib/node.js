...
classPrototype.fire = function (event) {
  ...
  (_states = this.states).emit.apply(_states, [event].concat(args));
};
...
classPrototype.action = function (action, rawFn) {
  for (var _len2 = arguments.length, states = Array(_len2 > 2 ? _len2 - 2 : 0), _key2 = 2; _key2 < _len2; _key2++) {
    states[_key2 - 2] = arguments[_key2];
  }

  var _states2;

  (_states2 = this.states).add.apply(_states2, [action].concat(states));
